...
class AccessToken {
	 ...
	 * @ignore
	 */
	function authorizeURL()    { return $this->host.'auth2/authorize'; }
	
	function __construct($client_id = null,$client_secret = null,$access_token=null){
		$this->client_id=$client_id;
		$this->client_secret=$client_secret;
		$this->access_token=$access_token;
	}

	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = false ) {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( is_array($token) && !isset($token['error_code']) ) {
			...
			//showmessage("get access token failed." . $token['error_code']);
		}
		return $token;
	}
