...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		if($fp = @fopen(WEB_ROOT.$tplfile, 'r')) {
			...
			fclose($fp);
		} elseif($fp = @fopen($filename = substr(WEB_ROOT.$tplfile, 0, -4).'.php', 'r')) {
		...
		if(!isset($langvar[$var])) {
			...
			if(!$isplugin) {
				...
				if(empty($_G['config']['plugindeveloper'])) {
					...
					if(@include(WEB_ROOT.'./data/plugindata/'.$vars[0].'.lang.php')) {
	...
	function loadcsstemplate() {
		...
		$content = @implode('', file(WEB_ROOT.'./data/cache/style_'.STYLEID.'_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen(WEB_ROOT.'./data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
