			...
			if (common.IsNullOrEmpty(json))
				...
				getConferenceWithYTX(jsonObj.id, function (status, result) {
					if (status == 1 && result.state == 1) {
						...
						conference.remove(jsonObj.id, function (state) {
							common.Write(res, req, {"status": 0, "msg": "会议已经删除"}, true, true);
						});
					}
				});
			}
		...
		common.GetReqResut(res, req, function (json) {
			if (common.IsNullOrEmpty(json))
				...
				async.eachSeries(jsonObj.users, function (key, next) {
					var userInfo = {"callid": "", "id": key.id, "number": key.number, "name": key.name, "mute": false, "state": 0};
				...
				conference.find(jsonObj.id, function (status, confInfo) {
					if (status == 1) {
						if (common.IsNullOrEmpty(confInfo.users))
							...
							if (common.IsNullOrEmpty(confInfo.callout)) {
								async.eachSeries(confInfo.users, function (key, next) {
									if (key.state == 0 && !common.IsNullOrEmpty(key.number) && common.IsNullOrEmpty(key.id)) {
										var conInfo = {"confid": jsonObj.id, "number": key.number, "role": 0};
										conInfo.joinurl = "join";
										var api = ytx.init(config.yuntongxun);
										api.inviteJoinConf(conInfo, function (err, re, data) {
									...
									conference.setAtt(jsonObj.id, true, function (status) {
										common.Write(res, req, null, true, true);
									});
								});
							}
							else
								common.Write(res, req, {"status": 0, "msg": "已经呼出所有外部手机用户"}, true, true);
						}
					}
					else
...
//呼叫加入会议某个用户的手机
function callConference(res, req) {
	var callback = qs.parse(url.parse(req.url).query).c;
	if (req.method != "POST") {
		common.Write(res, req, null, true, false);
	}
		...
		common.GetReqResut(res, req, function (json) {
			if (common.IsNullOrEmpty(json))
				...
				jsonObj = JSON.parse(json);
				var conInfo = {"confid": jsonObj.id, "number": jsonObj.number, "role": 0};
				conInfo.joinurl = "join";
		...
		common.GetReqResut(res, req, function (json) {
			if (common.IsNullOrEmpty(json))
				...
				async.eachSeries(jsonObj.users, function (key, next) {
					...
					conference.quit(jsonObj.id, jsonObj.users, function (status) {
						if (status == 1)
							common.Write(res, req, null, true, true);
						else
							common.Write(res, req, null, true, false);
			...
			if (common.IsNullOrEmpty(json))
				...
				async.eachSeries(jsonObj.users, function (key, next) {
					...
					api.confMute(conInfo, function (err, re, data) {
						conference.setMute(jsonObj.id, key.callid, true, function(ee){
							console.log(data);
							next();
						});
					});
			...
			if (common.IsNullOrEmpty(json))
				...
				async.eachSeries(jsonObj.users, function (key, next) {
					...
					api.confUnMute(conInfo, function (err, re, data) {
						conference.setMute(jsonObj.id, key.callid, false, function(ee){
							console.log(data);
							next();
						});
					});
...
exports.callConf = callConference
exports.quitConf = quitConference;
